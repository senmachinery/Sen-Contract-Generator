...
- **index.html — v13.17**: **New doc type "Agent Price List"** (catalog-style multi-model agent price list). Pick it in Doc Type → the form shows an **Agent Price List** panel (`#apl-panel`, English UI): per model name / subtitle / photo URL / key numbers (`value | unit | label` lines) / specifications (`label | value`) / standard equipment (one per line) / note / an editable price grid (click a cell); plus Optional items + AVR grids, terms, signature. Rendered by a **separate** `buildAgentPriceListHTML()` (page 1 letterhead + To/Attn + No./Date + title + contents; one page per model: photo, big key numbers, standard equipment, specs, price-by-colours table in navy; last page Optional Items + AVR + terms + SELLER signature). Data lives in **`state.apl`** (`aplNormalize` backfills defaults, called from `normalizeState` only when `apl` exists). Gates: `buildContractHTML` early-returns for this doc type (placeholder until "Start"); `isQuoteLike` + `docPrefix` treat it like a Quotation (**Q series**); `onDoctypeChange` keeps `/Q/`; `saveToSheets` saves amount 0 + title "Agent Price List — …" and **skips the auto follow-up**; export filename "… - Agent Price List"; **Excel/Word export blocked** (use Print/PDF). Photos must be Drive URLs (pasted `data:` rejected; `/file/d/ID` links auto-converted to thumbnail URLs). Verified: A) 5 script blocks `node --check` OK; B) 3 real Library states × 3 doc types (+apl:null) **18/18 byte-identical** v13.16 vs v13.17; C) Sungdo Korea list renders 8 pages in the real page (Edge headless), 6 photos load, no page errors; adversarial review (2 agents: code + 360 data cells) → fixes applied. Frontend-only, **no backend redeploy**. Archive: `SEN Contract Generator v13.17.html`. First doc: `SEN/Q/NR/26-2570` (Korea Sungdo).

- **index.html — v13.14**: **Split the Drive image library by machine type** so Parts/Accessories photos stop cluttering the machine-photo picker (Max: "配件照片跟机器照片分开、太多难找"). Added a **"Library" toggle (🖨️ Machines | 🔧 Parts)** at the top of the Pick-from-Drive modal. New JS: `IP_FOLDER_MACHINE='SenLabels_MachineImages'` / `IP_FOLDER_PARTS='SenLabels_PartsImages'` / `_pickerFolder`; `setPickerLibrary(lib,silent)` flips the folder + pill states + hint text + resets Status/Type filters (so a stale filter never shows an empty grid after switching). `openImagePicker()` auto-selects Parts when `M().machineType==='parts'`, else Machines. `searchImagePicker()` now sends `&folder=${_pickerFolder}` and **keys `_pickCache.img` by folder** (was single-cache → would show stale list across libraries). `deleteImageFromPicker` and `saveImagesToDrive` route to the active/type-matched folder. **NO backend redeploy** — `doGet` already passes `e.parameter.folder` to `listImages`, and `getOrCreateDriveFolder` auto-creates `SenLabels_PartsImages` on first list/upload. Verified: 4 script blocks `node --check` OK; grep-confirmed no stray hardcoded machine-folder left in picker paths. Frontend-only. Max must create/populate the `SenLabels_PartsImages` Drive folder (auto-created empty on first open).

- **index.html — v13.12**: **Bank-charges clause on the contract (v13.12)** — new toggle **"Buyer bears all bank charges"** in the Bank Details form (`f-buyer-bank-charges`, default **checked**). New state field `state.buyerBankCharges`: initial-state literal + `newContract()` reset = **true** (new contracts get it); `normalizeState` backfills **false** when undefined so **old saved contracts render byte-identical**. When `=== true`, `buildBankDetails()` appends a clause under BANK DETAILS: *"All bank charges, including any intermediary / correspondent bank fees, shall be borne by the Buyer, so that Sen Labels Machinery Sdn Bhd receives the full invoiced amount net of any deductions."* Wired into both `syncStateFromForm` copies + both load-JSON populate blocks; `newContract()` also resets the DOM checkbox to checked (adversarial-review fix — without it, an unchecked prior contract would leave DOM/state out of sync and silently drop the clause on the next contract). Scope = **PDF/preview only** (bank details were already PDF-only; Word/XLSX exports don't render a bank block). Verified: A) 4 script blocks `node --check` OK; B) `buildBankDetails` OFF **byte-identical** OLD vs NEW (vm+stub harness, feature off & undefined); C) ON adds exactly the clause (default + custom-bank branch). Surgical diff (23 insertions, 3 deletions). Frontend-only, **no backend redeploy**. Archive: `SEN Contract Generator v13.12.html`.

- **index.html - v12.6**: **English-only UI pass + parts-group Paste image + history note (v12.6)** - Converted ALL visible UI Chinese to English (~90 strings across 4 batches): tabs (Basic/Machine/Price/Extras/History), the v12.1-12.5 additions I had wrongly added in Chinese (rich-bar hints, picker Status/Type pills, color-button titles, no-image msg), section labels, the Share/Print dialog, the connect/offline card, Advanced-options labels, floor-price labels, key alerts, paste-status msgs. Left ONLY code comments + the `_ipParse` zh->type map (functional, matches the Chinese filename prefixes). Verified buildContractHTML body byte-identical pre/post -> **contract PDF output unchanged**. Added a **Paste image (Ctrl+V)** button to each parts group; made `_imgPasteBlob` parts-aware (routes paste to active group/item, mirroring selectImageFromPicker). **LESSON: never add Chinese to the app UI** (violates rule: UI is English; only chat with Max is Chinese) - I broke this in v12.1-12.5, fixed here. History tab = local recent contracts from localStorage.sen_history (empty until contracts are saved locally; not broken, populates with use - kept). Frontend-only, no backend redeploy. Archive: SEN Contract Generator v12.6.html.
- **index.html — v12.5**: **Pick-from-Drive image picker: Status + Type filter pills (like Machine DB) (v12.5)** — UI-only (does NOT touch buildContractHTML). Parses each image filename via new  (reads the  Chinese prefix added to the 94 SenLabels_MachineImages files, keyword fallback) → {status:New/Refurbished, type:Printing/Die Cut/Slitting/Cutter/Rewinder/Other}. Added pill rows +  +  (filters  by status+type, keeps original index so  still works).  now delegates grid render to . Verified: syntax OK,  unit test, surgical diff, 0 render-path changes. Frontend-only, no backend redeploy. Archive: SEN Contract Generator v12.5.html.
